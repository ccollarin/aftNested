# Construct outer monotone increasing B-spline basis

.build_nmi_bspline_basis <- function(object, data, knots, si){
  ...
  if( !is.null(object$kde) & (abs(mean(x)-0.5) > 1e-6 | min(x)<0 | max(x)>1) ){
    stop("The data should be uniform distributed!")
  }
  ...
  if( is.null(knots_x) ){
    ...
    if(is.null(object$kde)){
      kex <- c(-6,6)
    } else { kex <- c(0, 1)}
                # Inner knots go from a to b uniformly
    dx <- diff(kex) / (kin-1)
  ...
  # reparametrise X to impose that f'(x) < 0, i.e. f is monotone decreasing
  c <- ncol(out$X)
  Sigma <- 1*lower.tri(matrix(1, ncol=c,nrow = c),diag = TRUE)
  out$X <- out$X %*% Sigma
  P <- diff(diag(dsmo),difference=1)
  out$P[[1]] <- P
  out$S[[1]] <- crossprod(P)

  out$drop.coef <- TRUE
  out$xm <- colMeans(out$X[,-1])
  out$X <- sweep(out$X[,-1], MARGIN = 2, out$xm , "-")
  out$S[[1]] <- out$S[[1]][-1,-1]

  out$bs.dim <- out$bs.dim - ncon -1
  dsmo <- out$bs.dim
  ...
  out$X <- cbind(matrix(0, n, di), out$X)
  ...
  out$S <- list(rbind(cbind(matrix(0, di, di), matrix(0, di, dsmo)),
                      cbind(matrix(0, dsmo, di), out$S[[1]])))
  dtot <- dsmo + di
  out$bs.dim <- dtot
  out$null.space.dim <- dtot - out$rank
  out$rank <- out$rank
  out$D <- NULL
  out$df <- dtot
  ...
  out$xt$si <- si
  out$xt$basis <- .wrap_nested_basis(b = basis, P = NS %*% Sigma, Xth = Xth,
                                     drop.coef = out$drop.coef, xm = out$xm,
                                     add_slope = add_slope, unif = !is.null(object$kde))
  return(out)
}
#   sm <- aftNested:::.diagPen(X = out$X, S = out$S[[1]], out$rank)
#   # reparametrise X to impose that f'(x) < 0, i.e. f is monotone decreasing
#   c <- ncol(sm$X)
#   Sigma <- 1*lower.tri(matrix(1, ncol=c,nrow = c),diag = TRUE)
#   sm$X <- sm$X %*% Sigma
#   sm$S <- t(Sigma) %*% sm$S %*% Sigma
#
#   # Final model matrix includes inner and outer matrix (inner part first "di" columns, just dummy needed by mgcv)
...
#   # Store extra things needed later on.
#   out$xt$si <- si
#   out$xt$basis <- .wrap_nested_basis(b = basis, P = NS %*% sm$B %*% Sigma,
#                                      Xth = Xth, add_slope = add_slope)
#   return(out)
# }
